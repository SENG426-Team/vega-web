import {doPostWithToken} from '../BaseAPI.js';
...
export function secretHandler(secretData, token){
	...
	return doPostWithToken(url+"/api/venus/secret/add_secret", secretData, token);
}

export function fetchSecrets(userData, token){
	console.log("fetchSecrets", userData);
	return doPostWithToken(url+"/api/venus/secret/fetch_secrets", userData, token);
}

export function fetchSharedSecrets(userData, token){
	console.log("fetchShareSecrets", userData);
	return doPostWithToken(url+"/api/venus/secret/fetch_shared_secrets", userData, token);
}

export function shareSecret(secretData, token){
	console.log("shareSecret", secretData);
	return doPostWithToken(url+"/api/venus/secret/share_secret", secretData, token);
}

export function deleteSecret(secret_id, token){
	console.log("deleteSecret", secret_id);
	return doPostWithToken(url+"/api/venus/secret/delete_secret", secret_id, token);
}
